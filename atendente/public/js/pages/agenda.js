document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadGoogleCalendarStatus(){const status=await request('/api/google-calendar/status');q('googleCalendarStatus').innerHTML=status.connected?`<div class="alert alert-success">Google Calendar conectado: <strong>${esc(status.calendar_name||'Agenda principal')}</strong>. <a href="/integrations.html#integration-google-calendar">Gerenciar integração</a></div>`:`<div class="alert" style="background:#eff6ff;border-color:#bfdbfe;color:#1e40af;">Use a agenda interna normalmente ou <a href="/integrations.html#integration-google-calendar"><strong>conecte o Google Calendar</strong></a> para bloquear horários ocupados.</div>`;}
  ...
  (async()=>{try{const settings=await request('/api/settings');if(settings.business?.tipo_negocio!=='servicos'){location.replace('/dashboard.html');return;}await Promise.all([loadServices(),loadAppointments(),loadAvailability(),loadBlocks(),loadGoogleCalendarStatus()]);if(window.lucide)window.lucide.createIcons();}catch(err){console.error('[Agenda]',err);message('Não foi possível carregar a Agenda agora. Atualize a página e tente novamente.',true);}})();
});
